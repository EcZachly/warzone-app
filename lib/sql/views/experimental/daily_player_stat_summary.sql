create or replace view warzone.daily_player_stat_summary AS
    SELECT
       date_trunc('day'::text, gm.start_timestamp) AS game_day,
       COALESCE(gm.game_category, '(all)') as game_category,
       ...
       sum(gm.kills)::real / NULLIF(sum(gm.deaths), 0)::double precision                  AS kdr,
...
GROUP BY gm.game_category, date_trunc('day'::text, gm.start_timestamp), gm.query_username, gm.query_platform

ORDER BY gm.query_username, gm.query_platform,
         (date_trunc('day'::text, gm.start_timestamp));
