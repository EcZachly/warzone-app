...
CREATE OR REPLACE VIEW warzone.gamer_rolling_trends AS
    ...
    WITH valid_users AS (
       ...
       CASE WHEN LEAD(query_username, 1) OVER(PARTITION BY query_username, query_platform, game_category ORDER BY start_timestamp)  IS NULL THEN TRUE ELSE FALSE END as is_latest_game
    FROM rolling
         )

SELECT *, last_10_rolling_average_kdr/NULLIF(last_100_rolling_average_kdr, 0) AS heat_index_10_100,
