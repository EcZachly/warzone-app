...
WITH source AS (
    SELECT
            ...
            gm2.objective as helper_objective,
    ...
    FROM  warzone.gamer_matches gm
     ...
     with_teammates AS (
       ...
       helping_player_platform,
        ...
        CAST(SUM(gm.helper_kills) AS real) / NULLIF(CASE WHEN SUM(gm.helper_deaths) = 0 THEN 1 ELSE SUM(gm.helper_deaths) END, 0) AS helper_kdr,
       ...
       END, 0) as gulag_win_rate,
         ...
         CAST(SUM(gm.helper_gulag_kills) AS REAL)/
               NULLIF(CASE
                   WHEN SUM( CASE WHEN gm.helper_gulag_deaths <= 1 THEN gm.helper_gulag_deaths ELSE 0 END + gm.helper_gulag_kills) = 0 THEN 1
                   ELSE SUM( CASE WHEN gm.helper_gulag_deaths <= 1 THEN gm.helper_gulag_deaths ELSE 0 END + gm.helper_gulag_kills)
              END, 0) as helper_gulag_win_rate,
