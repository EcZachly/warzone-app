...
WITH agg AS (
    SELECT
           ...
           COALESCE(NULLIF(SUM(CASE WHEN gulag_deaths <= 1 AND gulag_kills <> 1 THEN gulag_deaths ELSE 0 END + gulag_kills)  ,0), 1)                                as gulag_win_rate,
           ...
           CAST(SUM(CASE WHEN team_placement = 1 THEN 1 ELSE 0 END) AS REAL)*100/COALESCE(NULLIF(COUNT(DISTINCT gm.match_id)  , 0), 1)      AS  win_percentage
    FROM warzone.gamer_matches gm
             JOIN warzone.matches_augmented m ON gm.match_id = m.match_id
    GROUP BY GROUPING SETS (
        (m.game_category, gm.query_username, gm.query_platform),
...
FROM agg a
        ...
        LEFT JOIN warzone.gamer_heat_ratings ghr
            ON a.username = ghr.query_username AND a.platform = ghr.query_platform AND ghr.game_category =  a.game_category
