...
async function selectAndInsert(){
    ...
    let users = await oldDb['warzone']['users'].find();

    let userLength = users.length;
    await insertPartitionedRecords(newDb, 'warzone', 'users', users, 1000, userLength, ['user_id'])


    let gamers = await oldDb['warzone']['gamer_relationships'].find();

    let gamerLength = gamers.length;
    await insertPartitionedRecords(newDb, 'warzone', 'gamer_relationships', gamers, 1000, gamerLength, ['user_id', 'username', 'platform'])
...
selectAndInsert();import oldDatabase from './database';
import newDatabase from './database_new';
