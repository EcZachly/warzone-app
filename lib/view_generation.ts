...
async function cleanAndRecreateViews() {
    logger.trace('clean and recreate views');
    await dropAllViews();
...
async function dropAllViews() {
    logger.trace('drop all views');

    const db = await database;

    const mappedQueries = coreViewNames.map((view) => `DROP VIEW IF EXISTS ${DATABASE_SCHEMA}.${view.split('.')[0]} CASCADE`).concat(
...
async function runViews(viewName) {
    logger.trace('run views: ' + viewName);
    const db = await database;
