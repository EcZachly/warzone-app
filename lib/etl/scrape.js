    ...
    return API.login(EMAIL, PASSWORD).then((response, api) => {
        ...
        return Bluebird.all(promises).then(
            (output) => {
                ...
                let matchPromises = mappedMatches.map((m)=> {return insertIntoDatabase(m, MATCH_TABLE)});
                return Bluebird.all(matchPromises).then((success)=>{
                    ...
                    let gamerMatchPromises = gamerMatches.map((m)=> {return insertIntoDatabase(m, GAMER_MATCH_TABLE)});
...
function run() {
    getGamer('fieldpipe', API.platforms.xbl)
