 ...
 * @returns {Promise<{gamer, matchHistory, queriedTimestamps, isBackfill}>}
 */
async function executePipeline(gamer, API) {
 ...
 */
export async function refreshData(query = {}) {
    let API = await login();
    let gamers = await queryGamers(query);
    await Bluebird.map(gamers, (gamer) => executePipeline(gamer, API), {concurrency: THREAD_CONCURRENCY_LIMIT});
